...
**Trạng thái:** Đã có Firestore + `RecyclerView` + tìm kiếm + nút thêm mẫu.
...
**Việc có thể làm thêm:** Màn chi tiết đơn, sửa trạng thái, thêm đơn thật (form), xóa đơn, lọc theo ngày, đồng bộ với Auth (chỉ user được phép ghi).
...
**Trạng thái:** Giống sản phẩm — khung rỗng.

| Thành phần | File |
|------------|------|
| Fragment | `LoHangFragment.java`, `fragment_lo_hang.xml` |

**Gợi ý:** Liên kết lô với sản phẩm / đơn nhập (ID tham chiếu), hạn sử dụng, số lô.
